#  site instituto
Para iniciar o projeto, algum membro do squad deve enviar o template inicial e fazer o primeiro commit para o branch main .

Crie um template de WebAPI na linguagem escolhida;
Crie um .gitignore para que não seja enviado arquivos necessários (cheque documentação, exemplos de outros projetos no github ou pesquisem como preferir);
Envie o template com o .gitignore para a branch main do repositório do seu squad.
Após o envio do modelo, as tarefas do projeto podem ser iniciadas.

Material de apoio para o Git/Github:

[Geral] Os comandos do Git | Documentação oficial https://git-scm.com/docs/git/pt_BR#_os_comandos_do_git

[Geral] APRENDA GIT E GITHUB DO ZERO - guia completo | Fernanda Kipper https://www.youtube.com/watch?v=pyM5QLS2h6M

[Ramos] GIT BRANCH E GIT MERGE - Git e GitHub #03 | Gustavo Caetano https://www.youtube.com/watch?v=gcOhd8rvBOc

[Pull Request] COMO FAZER UM PULL REQUEST - Git e GitHub #04 | Gustavo Caetano https://www.youtube.com/watch?v=IMerCpaT_zM

[Geral] Git e Github para Iniciantes | Willian Justen [Playlist] https://www.youtube.com/watch?v=IBClN6VpJDw&list=PLlAbYrWSYTiPA2iEiQ2PF_A9j__C4hi0A
